...
void Sentence::complete(const Dict &d)
{
	...
	{
		h_distance = this->completion_hammer(d.get_sentence(i));
		/*cout << "h_distance: " << h_distance << endl;
		cout << "d.get_sentence(i): " << d.get_sentence(i) << endl;*/
		...
		for(int j = 0; j < 10; j++)
		{
			if(h_distance > para_completions[j])
				...
				if(h_distance < para_completions[j])
				{
					...
					completions[j] = d.get_sentence(i);
					break;
				}
				else
          			if(h_distance == para_completions[j])
          			{
            			if(d.get_sentence(i) < completions[j])
            			{
                 		  //makes space
				      		for(int k = 8; k >= j; k--)
				     		{
				      		...
				      		//puts new sentence and distance in space
				      		para_completions[j] = h_distance;
				      		completions[j] = d.get_sentence(i);
...
void Sentence::check(const Dict &d)
{
  ...
  cout << input.length() << endl;
	for(int i = 0; i < d.get_sentence_count(); i++)
	{
		...
		h_distance = this->correction_hammer(d.get_sentence(i));
    //if(h_distance <= 0) continue;
      ...
      if(h_distance > para_corrections[j])
        ...
        if(h_distance < para_corrections[j])
        {
				  ...
				  //puts sentence and distance in new space
				  para_corrections[j] = h_distance;
				  corrections[j] = d.get_sentence(i);
          break;
          ...
          if(h_distance == para_corrections[j])
          {
            if(d.get_sentence(i) < corrections[j])
            {
                 	  //makes space
				      for(int k = 8; k >= j; k--)
				      {
...
int Sentence::completion_hammer(string w)
{
...
int Sentence::correction_hammer(string w)
{
	int difference = 0;
...
bool Sentence::has_same_start(string s1, string s2)
{
